...
let customerPopup = new (function () {

    let self = null;
    let _args = null;

    //UI
    this.popup_id      = "#popup-customer";
    ...
    this.open = function (popupMode, dataSelection, fn) {
        ...
        if (popupMode === POPUP_MODE.EDIT) {
            ...
            {
                customerPopupPopup.callBack(this.jsonData);
                return;
            }
            this.bindDom(this.jsonData);
        ...
        if (popupMode === POPUP_MODE.DELETE) {
            ...
            if (!dataSelection) //validate selected item
            {
                customerPopup.callBack(this.jsonData);
                return;
            }
    ...
    this.OnExcelExportClick = function (args) {
        let gridObj = document.getElementById("grdCustomer").ej2_instances[0];
        if (args.item.id === 'grdCustomer_excelexport') {
            gridObj.excelExport();
        }
    }
