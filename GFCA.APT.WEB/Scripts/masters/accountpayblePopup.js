...
let accountpayblePopup = new (function () {

    let self = null;
    let _args = null;

    //UI
    this.popup_id = "#popup-accountpayble";
    ...
    this.open = function (popupMode, dataSelection, fn) {
        ...
        if (popupMode === POPUP_MODE.EDIT) {
            ...
            {
                accountpayblePopup.callBack(this.jsonData);
                return;
            }
            this.bindDom(this.jsonData);
    ...
    this.OnExcelExportClick = function (args) {
        let gridObj = document.getElementById("grdAccountpayble").ej2_instances[0];
        if (args.item.id === 'grdAccountpayble_excelexport') {
            gridObj.excelExport();
        }
    }
